...
//        dd($carts);
    ...
    public function addToCart($id){
        ...
        if ($item_exits > 0) {
            return response()->json(['message'=>'มี item ในตระกร้าแล้ว','status'=>302]);
        }
        $data=[
    ...
    public function clearCart() {
        ...
        return response()->json(['message'=>'Clear ตะกร้าเรียบร้อยแล้ว','status'=>200]);
    }

    public function createOrder() {
        $carts = Cart::Where('user_id',auth()->user()->id)->get();
        if($carts->isEmpty()){
            return Response()->json(['message'=>'ไม่ item ในตะกร้า','status'=>422]);
        }
